# Cloudflare Worker + D1 导航站 部署指南

> 本项目同时支持两套环境：
> - **本地开发**：Node.js + Express + MySQL（见项目根目录）
> - **线上部署**：Cloudflare Worker + D1（本文档）

## 新功能 (v2.0)

1. **拖动排序** — 分类和链接均支持拖拽排序，跨分类移动链接
2. **自动补全 https** — 输入 `example.com` 自动转为 `https://example.com`
3. **搜索优化** — 实时过滤分类和链接，无结果时自动隐藏分类
4. **安全加固** — 移除 URL 参数认证，仅用 Authorization 头
5. **导入导出** — JSON 格式，与 Node.js 版 / Cloudflare 版互相兼容
6. **暗色/自动主题** — 浅色/深色/跟随系统

---

### 第一步：创建 D1 数据库

Cloudflare 控制台 → Workers & Pages → D1 → 创建数据库（名称如 `my-nav-db`）

进入 D1 Console，执行以下 SQL：
...
Cloudflare 控制台 → Workers & Pages → 创建 → 创建 Worker（名称如 `my-nav`）

将以下完整代码粘贴到编辑器中，点击"保存并部署"：

```javascript
/**
 * Cloudflare Worker + D1 导航站 v2.0
 * 功能：分类/链接CRUD、拖拽排序、跨分类移动、搜索、主题切换、导入导出
 */
export default {
...
Node.js 版（MySQL）和 Cloudflare 版（D1）的导出 JSON 格式完全兼容，可互相导入。

Node.js 版导出后导入到 Cloudflare：
1. 本地 Node.js 版登录 → 导出备份 → 下载 JSON
2. Cloudflare 版登录 → 导入恢复 → 选择 JSON 文件
